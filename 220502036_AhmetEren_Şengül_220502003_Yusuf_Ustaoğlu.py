...
class Truck:
    ...
    def createTruckDict(self):  # Csv dosyasını sözlüğe çevirme.
        with open(self.file, "r", encoding="utf-8") as file:  # utf-8 özelliği eklendi
...
class Stack:
    ...
    def __len__(self):  # Yığının uzunluğunu alma
        return len(self._theItems)

    def peek(self):  # En üstteki değere bakma.
        assert not self.isEmpty(), "Boş bir yığında peek işlemi yapılamaz"
        return self._theItems[-1]

    def pop(self):  # Üstteki değeri çıkartma.
        assert not self.isEmpty(), "Boş yığında pop işlemi yapılamaz."
